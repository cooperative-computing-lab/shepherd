#!/usr/bin/env python3
"""Visualize Shepherd workflow manager configurations and state transitions."""
...
def render_dot(
    dot_source, output_filename="workflow_visualization", output_format="png"
):
...
def generate_state_times_graph(
    state_transition_times, output_prefix, output_format="png", fig_size=(15, 12)
):
    sorted_state_transition_times = dict(
        sorted(
            state_transition_times.items(), key=lambda x: x[1]["started"], reverse=True
        )
    )
    ...
    unique_states = set(
        {
            state
            for times in state_transition_times.values()
            for state in times
    ...
    color_map = dict(zip(unique_states, state_colors, strict=False))

    fig, ax = plt.subplots(figsize=fig_size)

    for idx, (_service, state_times) in enumerate(
        sorted_state_transition_times.items()
    ):
        ...
        for state, state_time in sorted_states[2:]:
            ...
            ax.broken_barh(
                ...
                (idx - 0.4, 0.8),
                facecolors=color_map[previous_state],
...
def generate_state_transition_graph(
    config, state_transition, output_prefix, output_format="png"
):
    ...
    for service, details in config["tasks"].items():
        service_type = details.get(
            "type", "action"
        )  # Default to 'action' if type is not specified
        node_color = colors.get(
            service_type, "lightgrey"
        )  # Default color if no specific type is found
...
def generate_workflow_graph(config, output_prefix, output_format="png"):
    ...
    for service, details in config["tasks"].items():
        service_type = details.get(
            "type", "action"
        )  # Default to 'service' if type is not specified
        node_color = colors.get(
            service_type, "lightgrey"
        )  # Default color if no specific type is found
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate visualizations for Shepherd workflow manger."
    )
    parser.add_argument(
        "--config", "-c", type=str, help="Path to the program config YAML file"
    )
    parser.add_argument(
        "--state_transition",
        "-s",
        type=str,
        help="Path to the state transition JSON file",
    )
    parser.add_argument(
        "--output_prefix",
        "-p",
    ...
    parser.add_argument(
        ...
        "-v",
        action="version",
        version=f"Shepherd Visualization Tool {__version__}",
    ...
    if config is not None and state_transition is not None:
        generate_state_transition_graph(
